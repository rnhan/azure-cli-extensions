...
@register_command(
    "container-storage pool volume update",
)
class Update(AAZCommand):
    """Update a Volume

    :example: Update a Volume.
        az container-storage pool volume update -g "rg" --pool-name "pool" -n "volume" --set capacityGiB=3 --tags "{key2011:cccc}"
    """
...
class _UpdateHelper:
    ...
    def _build_schema_volume_read(cls, _schema):
        ...
        labels = _schema_volume_read.properties.labels
        labels.Element = AAZStrType()

        status = _schema_volume_read.properties.status
        status.message = AAZStrType()
        status.state = AAZStrType(
            flags={"required": True},
        )

        volume_type = _schema_volume_read.properties.volume_type
        volume_type.elastic_san = AAZObjectType(
            serialized_name="elasticSan",
        )

        elastic_san = _schema_volume_read.properties.volume_type.elastic_san
